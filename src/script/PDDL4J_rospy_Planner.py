...
#########################################################################
	...
	def launch(self):
		try:
			if self.sync == "sync":
				...
				print("Usage : rosrun pddl4j_rospy PDDL4J_rospy_ros_node.py [sync]")
	...
	def listenerDomainNameProblem(self):
	    ...
	    :return: void
		'''
		rospy.Subscriber("domain_problem_from_controller_topic", String, self.resolvProblemAsTopic)
	...
	def talkerJsonObject(self, jsonPath, problemResolved, problemDirectory, problemName, jsonPythonObject, operationStatus):
		'''
		...
		if (problemResolved):
			...
			print("The problem has not been resolved...\nSending result to the controller...")
			if not rospy.is_shutdown():
				str_sent = jsonPath + "___False___" + problemDirectory + "___" +  problemName + "___" + jsonPythonObject + "___" + operationStatus
	...
	def serverDomainNameProblem(self):
	    ...
	    :return: void
		'''
		s = rospy.Service('serverDomainNameProblem', RequestPlannerPlanification, self.resolvProblemAsService)
	...
	def resolvProblemAsTopic(self, req):
		...
		self.talkerJsonObject(returnData[0], returnData[1], returnData[2], returnData[3], returnData[4], returnData[5])
